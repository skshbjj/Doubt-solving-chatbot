...
def get_bot_response():
    ...
    cur.execute(" INSERT INTO yt(question) VALUES(%s)",[userText]) ## !! Put your table name here
    mysql.connection.commit()
    cur.close()
    compare =userText.lower()
##    if (compare=="ask me a question"):
##        return mcq()
        
    cur = mysql.connection.cursor()
    cur.execute('''Select id ,question,answer from quora''')  ## !! Put your table name here
    highScore=0
    z=0
    highScoreAnswer="<p>I can't understand what you are saying</p>"
    rv = cur.fetchall()

    for x in rv:
        a =  userText
        score = SequenceMatcher(None,a,x[1]).ratio()
        if (score>0.6):
            if (highScore<score):
                highScore = score
                highScoreAnswer = x[2]
                z=1
    if z==1:
        z= html2text.html2text(highScoreAnswer)
        tts = gTTS(text=z, lang='en')
        tts.save("hello.mp3")
        os.system('python new.py')  ## !! Put your path for new.py here
        return highScoreAnswer
    else:
        return reddit(userText)
...
def mcq():
    ...
    query_string='''Select id ,question,opt1,op2,op3,op4,correct from mcq where id = %s'''  ## !! Put your table name here
    cur.execute(query_string, (rand,))
    ques = cur.fetchall()
...
def getyt():
    cur = mysql.connection.cursor()
    cur.execute('''Select question from yt ORDER BY timeStamp DESC LIMIT 1''')     ## !! Put your table name here
    search = cur.fetchall()
    print(search)
    z=re.sub(r'\W+', " ", search[0][0])
    g= z.replace(" ", "+")
    my_url ="https://www.youtube.com/results?search_query="+ g
    uclient = uReq(my_url)
    page_html = uclient.read()
    uclient.close()
    page_soup = BeautifulSoup(page_html,"html.parser")

    div = page_soup.findAll("div",{"id":"content"})
    udiv = div[0].findAll("div",{"class":"yt-lockup-content"})
    uudiv = udiv[0].findAll("a")

    output = uudiv[0]['href']
    k = output.strip('/watch?v=')

    div = page_soup.findAll("div",{"id":"content"})
    udiv = div[0].findAll("div",{"class":"yt-lockup-content"})
    uudiv = udiv[1].findAll("a")
    output = uudiv[0]['href']
    l = output.strip('/watch?v=')
    div = page_soup.findAll("div",{"id":"content"})
    udiv = div[0].findAll("div",{"class":"yt-lockup-content"})
    uudiv = udiv[2].findAll("a")
    output = uudiv[0]['href']
    m = output.strip('/watch?v=')
